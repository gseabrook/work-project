...
drop table if exists mandate_party CASCADE;
...
create table customer (id_type integer, id_value varchar(255), name varchar(255), phone_number varchar(255), principal_uid varchar(255), id integer not null, primary key (id));
...
create table mandate (id SERIAL UNIQUE, amount decimal(19,2) not null, status integer, frequency integer, reference_number varchar(255), fpx_transaction_id varchar(255), purpose_of_payment varchar(255), maximum_frequency integer, registration_date timestamp, buyer_id integer not null, merchant_id integer not null, customer_bank_account_id integer, node_id varchar(255), date_created timestamp, primary key (id));
create table mandate_party (id SERIAL UNIQUE, email_address varchar(255), phone_number varchar(255), primary key (id));
create table merchant (company_name varchar(255), company_registration_number varchar(255), logo_file_location varchar(255), id integer not null, primary key (id));
...
alter table mandate add constraint FKrblr9jysrw4jmgs8it3s1rjq3 foreign key (buyer_id) references mandate_party;
alter table mandate add constraint FK4x77j3hw2seixa474xtcvp55i foreign key (merchant_id) references mandate_party;
